...
-----------------------------------------------------------------------------------
VERSÃO DIDÁTICA 2025 — COMPLETA COM EXCLUSÃO E RELATÓRIO
===================================================================================
*/
...
void gerar_arquivo_texto(FILE *arq) {
    ...
    fprintf(txt, "================ RELATÓRIO DE ALUNOS ================\n\n");

    while (fread(&aluno, sizeof(reg), 1, arq) == 1) {
        fprintf(txt, "Registro %d\n", index);
        fprintf(txt, "Nome......: %s\n", aluno.nome);
        fprintf(txt, "Matrícula.: %d\n", aluno.matricula);
        fprintf(txt, "Nota......: %.2f\n", aluno.nota);
        fprintf(txt, "Status....: %s\n\n", aluno.status == '*' ? "EXCLUÍDO" : "ATIVO");
        index++;
    }
...
void excluir(FILE *arq) {
    ...
    printf("\nInforme o código do aluno para excluir: ");
    if (scanf("%d", &cod) != 1) {
        printf("Entrada inválida!\n");
        limpaBuffer();
        return;
    ...
    if (toupper(conf) == 'S') {
        ...
        fwrite(&aluno, sizeof(reg), 1, arq);
        fflush(arq);
        printf("Registro excluído logicamente!\n");
    } else {
